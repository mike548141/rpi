...
### Changed
- **The roadmap is now one file per item, with a generated index** (docs only; no code change). The monolithic
  root `ROADMAP.md` became `docs/roadmap/` — 7 sections, 29 item files (27 migrated, plus this item and its review pointer), each opening with its checkbox state
  line — and `docs/ROADMAP.md` is a ~70-line index generated by atelier's `../atelier/tools/board.py` and enforced by the
  `board` floor check on both planes. Adopting the parent estate's board-store decision (2026-08-15) on Mike's
  instruction. This repo's board was never big enough for the read-cost argument to bite; what applied here was
  *state asserted rather than derived* — status lived in prose glyphs (✅ / ◑) no scanner could read — and
  contention at file grain. Each item's own `git log` is now its provenance. Every top-level bullet was given a
