...
- **SQLite persistence.** `--save-db [PATH]` appends the run to a local SQLite database (default
  ...
  never changes the card's exit code. The local seed of the crowd-sourced database (upload still gated on a
...
## Post-0.9 (unreleased) — macOS UX + CSD liar-checks

- **macOS: auto-detect the inserted card.** With no `--device`/`--dir`, `gather_macos()` scans `diskutil` for a
  removable/external whole disk (preferring an SD-bus reader), profiles that card instead of the boot disk, and
  points the benchmark/sweep at its mount point; the selection is announced. `macos_card_identity()` also asks
  `system_profiler SPCardReaderDataType` for a built-in SD slot's real card product/make/serial, filling gaps
  `diskutil` leaves blank. USB readers present as generic mass storage. New `tests/test_macos.py`.
- **CSD structural liar-checks.** `cross_check()` flags a structurally malformed register (reserved structure
  version, zero/undefined TRAN_SPEED, empty or mandatory-class-missing command classes, illegal READ_BL_LEN) —
  a spec-valid register is what a genuine controller emits, so garbage is a counterfeit tell (all `warn`).
- **Bus-ceiling explainer (info, not fail).** When a card's rated class needs more sustained write than its CSD's
  advertised bus can carry, an `info` note explains the card is bus-limited on a non-UHS host — so a genuine card
  measuring below its label is understood, not mistaken for a fake. The decision *not* to infer a fake from
  TRAN_SPEED is recorded in ADR [0003](decisions/0003-tran-speed-is-not-proof-of-fake.md).
